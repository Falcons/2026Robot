...
public class TurretSim extends SubsystemBase {
    ...
    public void periodic() {
      ...
      SmartDashboard.putNumber("Turret/TurretSim/globalAngle", Math.toDegrees(getGlobalRad2()));
     ...
     */
    public double getGlobalRad1() {
      LaunchCalculator.getInstance().clearLaunchingParameters();
      return LaunchCalculator.getInstance().getParameters(swerve, shooterSim.getShooterRPS(), turretEncoderSim.getPosition()).turretAngle().getRadians();
    }

    /**
     * @return the angle in which the shooter should be aiming at towards the goal in radians -pi to pi
     */
    public double getGlobalRad2() {
      LaunchCalculator2.getInstance().clearLaunchingParameters();
      return LaunchCalculator2.getInstance().getParameters(swerve, turretEncoderSim.getPosition()).turretAngle().getRadians();
    }
    ...
    public double getRelativeRad() {
      return MathUtil.angleModulus(getGlobalRad2() - swerve.getHeading().getRadians());
    }
    ...
    public boolean inRange() {
      return getGlobalRad2() - getRelativeRad() < MovementConstants.turretError;
    }
